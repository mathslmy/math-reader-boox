 ...
 *
 * 职责：
 * 1. 自动探测 PWA 中当前"可手写"的画布（习题作答 / PDF 批注 / 草稿纸 / 笔记 /
 ...
 *    PWA 自己的绘制/撤销/保存/识别逻辑零修改全部复用。
 * 3. 拦截 blob: 下载（PWA 导出 PDF/JSON），转交原生保存到下载目录。
 *
 * index.html 与上游 math-reader 保持字节一致，升级时直接覆盖 assets/www 即可。
 */
(function () {
    'use strict';
    ...
    window.__booxPen = { active: hasNative, onStroke: function () {} };
    if (!hasNative) {
    ...
    var CONFIGS = [
        ...
        { name: 'note',         id: 'noteCanvas',           cssWidth: 2,
          eraserBtn: 'noteEraserBtn',   eraserToggle: 'toggleNoteEraser' },
